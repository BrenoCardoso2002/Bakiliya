...
class Register_Empresa : AppCompatActivity() {
    ...
    private lateinit var inputCNPJ: TextInputEditText
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        inputNome = findViewById(R.id.Input_NomeFantasia_RegisterEmpresa)
        inputCNPJ = findViewById(R.id.Input_CNPJ_RegisterEmpresa)
        inputEmail = findViewById(R.id.Input_Email_RegisterEmpresa)
        inputCEP = findViewById(R.id.Input_CEP_RegisterEmpresa)
        inputNRua = findViewById(R.id.Input_Numero_RegisterEmpresa)
        inputSenha = findViewById(R.id.Input_Senha_RegisterEmpresa)
        inputRepSenha = findViewById(R.id.Input_RSenha_RegisterEmpresa)
        txtInfoCEP = findViewById(R.id.TextView_DadosCEP_RegisterEmpresa)
        bVerMais = findViewById(R.id.Button_VerMais_RegisterEmpresa)
        bCadastrar = findViewById(R.id.Button_Cadastrar_RegisterEmpresa)
        bJTConta = findViewById(R.id.Button_JtConta_RegisterEmpresa)

        // Chama a função que fecha o teclado:
        closeKeyBoard()

        // Primeiro código:
        // Clique do botão para se cadastrar:
        bCadastrar.setOnClickListener {
            ...
            // Obtem o texto das caixas de textos:
            val nomeS = inputNome.text.toString().trim()
            val cnpjS = inputCNPJ.text.toString().trim()
            ...
            if(nomeS.isEmpty() || cnpjS.isEmpty() || emailS.isEmpty() || cepS.isEmpty() || ruaS.isEmpty() || senhaS.isEmpty() || repsenhaS.isEmpty()){
                ...
                alertError(descripton)
            }else if(!validaCNPJ(cnpjS)){
                val descripton = getString(R.string.o_cnpj_fornecido_inv_lido_ndeve_estar_no_seguinte_formato_nxx_xxx_xxx_0001_xx)
                alertError(descripton)
    ...
    private fun alertError(descripton: String) {
        ...
        val dialogBinding = layoutInflater.inflate(R.layout.error_in_register_empresa, null)
        val myDialog = Dialog(this)
        myDialog.setContentView(dialogBinding)
        myDialog.setCancelable(true)
        myDialog.window?.setBackgroundDrawable(ColorDrawable(Color.TRANSPARENT))
        myDialog.show()
        myDialog.findViewById<TextView>(R.id.TextView_DescriptionError_RegisterEmpresa).text = descripton
        myDialog.findViewById<Button>(R.id.Button_OK_erroRegisterEmpresa).setOnClickListener {
            myDialog.dismiss()
        }
    }

    fun validaCNPJ(cnpj: String): Boolean{
        if(!verificaInternet()){
            alertaSemConexao()
        }
        // XX. XXX. XXX/0001-XX
        val padrao = Regex("^\\d{2}\\.\\d{3}\\.\\d{3}/\\d{4}-\\d{2}$")
